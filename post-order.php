 ...
 * Description: A lightweight plugin that adds the option to reverse the post order for a specific category to be date ascending. 
 When creating or editing a category from the Admin Dashboard, the user can choose to sort the posts for that category by oldest or newest (default WordPress sort).
 Useful for journals, books or achives, who need to have a chronological sort order for certain category archive's pages.
 * Version: 1.0
...
function poc_changePostOrder(){
    ...
    <tr class="form-field">
        ...
        <select name="post-order" id="post-order">
            <option <?php if ($post_order == 'newest') { ?>selected="true" <?php }; ?>value="newest"><?php _e('Newest'); ?></option>
            <option <?php if ($post_order == 'oldest') { ?>selected="true" <?php }; ?>value="oldest"><?php _e('Oldest'); ?></option>
        </select>
        </td>
    </tr>
